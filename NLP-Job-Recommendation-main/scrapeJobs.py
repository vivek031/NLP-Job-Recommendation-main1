...
#//driver_path = "C:/Users/vivek/OneDrive/Desktop/All folder/8th sem/project/NLP-Job-Recommendation-main/chromedriver_win32/chromedriver"
...
def do():
    ...
    db = client['jobs']
    mydb = db['narkuri']  # Collection creation moved outside the loop
    ...
    for search in skills:
        try:
            ...
            experiences = soup.find_all("li", {"class": "experience"})
            salaries = soup.find_all("li", {"class": "salary"})
            locations = soup.find_all("li", {"class": "location"})
            skills_lists = soup.find_all("ul", {"class": "tags"})

            for i in range(min(len(titles), len(experiences), len(salaries), len(locations), len(skills_lists))):
                job_data = {
                    'title': titles[i].text.strip(),
                    'url': titles[i].get('href').strip(),
                    'experience': experiences[i].text.strip(),
                    'salary': salaries[i].text.strip(),
                    'location': locations[i].text.strip().split(', '),
                    'skills': [skill.text.strip() for skill in skills_lists[i].find_all('li')],
            ...
            if data:
                ...
                print("Data inserted for search:", search_title)
    ...
    if ex:
        print("Exceptions occurred:", ex)
    if wrong:
        print("Wrong URLs:", wrong)
